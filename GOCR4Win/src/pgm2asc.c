...
void measure_pitch( job_t *job ){     /* word spacing */
  int numdists=0, spc=0,              /* number of stored distances */
      ...
      monospaced=0, l1, char_width_min=1023, char_width_max=0;
  ...
  { /* 0 means all lines */
    ...
    if (job->cfg.verbose)
      fprintf(stderr, " num_gaps= %2d x_width_min= %2d max= %2d",
              numdists, char_width_min, char_width_max);
    if (numdists<8) {
...
int list_insert_spaces( pix *pp, job_t *job ) { 
  ...
  i=0;
  for_each_data(&(job->res.boxlist)) {
    int thispitch=0, thismono=0, mdist=0, pdist=0; // spacing paras per line
    box2 =(struct box *)list_get_current(&(job->res.boxlist));
    cc=0;
    ...
    if((box3 = (struct box *)list_prev(&(job->res.boxlist), box2))){
      ...
      if (box2->line<=maxline && cc==0) {  // lines and chars must be sorted!
        mdist = (box2->x1 + box2->x0 - (box3->x1 + box3->x0) + 1)/2;
        pdist  = box2->x0 - box3->x1 + 1;
        thispitch = job->res.lines.pitch[box2->line];
        thismono  = job->res.lines.mono[box2->line];
        // JS-2010-09: causing problems with prop font around 'I' in INVALID
        ...
        if ((thismono!=0 &&   mdist >= thispitch) // 2010-09-24 ???
